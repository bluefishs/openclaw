...
export function createGatewayCloseHandler(params: {
  ...
  /** Optional: gstack services to shut down (Redis connections, SSE, timers) */
  gstackCleanup?: Array<{ shutdown: () => Promise<void> }>;
}) {
  return async (opts?: { reason?: string; restartExpectedMs?: number | null }) => {
    try {
      ...
      // Shut down gstack services (EventRelay SSE, TaskTracker Redis, ConversationMemory Redis)
      if (params.gstackCleanup) {
        await Promise.allSettled(params.gstackCleanup.map((svc) => svc.shutdown()));
      }
