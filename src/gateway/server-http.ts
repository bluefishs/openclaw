...
export function createGatewayHttpServer(opts: {
  ...
  /** Optional: gstack multi-agent endpoints (/delegate, /tasks/*, /events) */
  gstack?: GstackHttpStagesConfig;
}): HttpServer {
  const {
    canvasHost,
    clients,
  ...
  async function handleRequest(req: IncomingMessage, res: ServerResponse) {
    ...
    try {
      ...
      // gstack multi-agent endpoints: /delegate, /tasks/*, /events
      if (gstack) {
        requestStages.push(...buildGstackRequestStages(req, res, gstack));
      }
