...
const lazyWorkflows = createLazy(() => import("./views/workflow-dashboard.ts"));
const lazyWorkflowActions = createLazy(() => import("./controllers/workflow-actions.ts"));
...
/** Convert ws:// or wss:// gateway URL to http:// or https:// for REST calls. */
        ...
        ${
          state.tab === "overview"
            ? renderOverview({
                ...
                metricsProps: {
                  metrics: state.metricsResult ?? null,
        ...
        ${
          state.tab === "workflows"
            ? lazyRender(lazyWorkflows, (m) =>
                m.renderWorkflowDashboard({
                  instances: state.workflowInstances,
                  events: state.workflowEvents,
                  definitions: state.workflowDefinitions,
                  connected: state.workflowConnected,
                  onResume: (wfId) => {
                    const actions = lazyWorkflowActions();
                    if (!actions) {
                      return;
                    }
                    const httpBase = wsUrlToHttp(state.settings.gatewayUrl);
                    void actions.resumeWorkflow(httpBase, state.settings.token, wfId);
                  },
                  onCancel: (wfId) => {
                    const actions = lazyWorkflowActions();
                    if (!actions) {
                    ...
                    const httpBase = wsUrlToHttp(state.settings.gatewayUrl);
                    void actions.cancelWorkflow(httpBase, state.settings.token, wfId);
                  },
                  onDeleteDefinition: (defId) => {
                    const actions = lazyWorkflowActions();
                    if (!actions) {
                      return;
                    }
                    const httpBase = wsUrlToHttp(state.settings.gatewayUrl);
                    void actions
                      .deleteWorkflowDefinition(httpBase, state.settings.token, defId)
                      .then((result) => {
                        if (result.success) {
                  ...
                  onExportDefinitions: () => {
                    const json = JSON.stringify(state.workflowDefinitions, null, 2);
                    const blob = new Blob([json], { type: "application/json" });
                    const url = URL.createObjectURL(blob);
                    const a = document.createElement("a");
                    a.href = url;
                    a.download = "workflow-definitions.json";
                    a.click();
                    URL.revokeObjectURL(url);
                  },
                  onImportDefinitions: (json) => {
                    const actions = lazyWorkflowActions();
                    if (!actions) {
                      return;
                    }
                    ...
                    try {
                      ...
                      for (const def of defs) {
                        void actions.createWorkflowDefinition(
                          httpBase,
                          state.settings.token,
                          def as Parameters<typeof actions.createWorkflowDefinition>[2],
                        );
                      }
                    } catch {
